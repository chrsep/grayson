// Handle image related logic on client side
export const uploadImage = async (image: File): Promise<string | null> => {
  const presignedUrl = await fetch("/api/images/presignedUrl", { method: "POST" })
  if (!presignedUrl.ok) return null
  ...
  return !uploadImage.ok ? null : key
}
