...
async function handleGuestPutLineItems(
  ...
  if (cart?.user !== null) return { status: 401, body: {} }
...
async function handleUserPutLineItems(
  ...
  if (!session.user?.email) return { status: 404, body: {} }

  let cart = await findCartByUserEmail(session.user.email)
  if (!cart) return { status: 404, body: {} }
...
const put: NextApiHandler = newMutationHandler(PutBody, async (body, session, req) => {
  ...
  if (!session) return handleGuestPutLineItems(cartId as string, body)
  return handleUserPutLineItems(session, body)
})
